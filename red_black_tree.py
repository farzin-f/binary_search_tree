...
class RedBlackTreeMap(TreeMap):
  ...
  """def impression(self):
    #Afficher les cles d'un arbre en ordre Breadth-first
    pp = ""  
    if not self.is_empty():
      fringe = LinkedQueue()             
      fringe.enqueue(self.root())
      pp += "Breadth-First Traversal:\n\n"
      i = 0
      while not fringe.is_empty():
        ...
        if self.num_children(p) > 0:
          ...
          if self.right(p) is not None:
            pp += " <--(" + str(self.right(p).key()) + ") - niveau " + str(self.depth(p)) + "\n"
          elif self.right(p) is None:
            pp += " <--( ) - niveau " + str(self.depth(p)) + "\n"
        else:
          pp += "( )--> " + str(p.key()) + " <--( ) - niveau " + str(self.depth(p)) + "\n"
    return pp"""
